...
public class ObjectPool<T> where T : MonoBehaviour
{
    ...
    {
        this.prefab = prefab;
        this.parent = new GameObject("ObjPool - " + prefab.GetType().ToString()).transform;
        this.parent.gameObject.hideFlags = HideFlags.HideAndDontSave;
    ...
    public List<T> Get(int count)
    {
        ...
        else
        {
            ...
            for (int i = 0; i < needToCreate; i++)
                objs.Add(GameObject.Instantiate(prefab, parent));
        }
        return objs;
    }
